...
const registerSchema = Joi.object({
    ...
    phone: Joi.string().pattern(/^[0-9+\-\s()]+$/).min(10).max(20).messages({
...
const courseSchema = Joi.object({
    ...
    courseType: Joi.string().valid('academic', 'professional', 'short-term', 'both').default('academic').messages({
        'any.only': 'Course type must be academic, professional, short-term, or both'
    }),
