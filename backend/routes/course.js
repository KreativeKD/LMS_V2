...
const ensureInstructorFallback = (course) => {
  ...
  if (
    !course.instructor &&
    Array.isArray(course.assignedTeachers) &&
...
const isAssociatedTeacherForCourse = (course, user) => {
  ...
  const isInstructor =
    course.instructor && String(course.instructor) === userId;
  const isAssigned =
    Array.isArray(course.assignedTeachers) &&
...
const serializeTestimonial = (testimonial) => {
  ...
  const overallRating = Number(
    testimonial.overallRating || testimonial.rating || 0,
  );
...
const isValidHalfStepRating = (value) =>
...
const getAccessibleCourseAndUnit = async (courseId, unitId, user) => {
  ...
  if (!canAccessCourseContent(course, user)) {
    return {
      error: {
        ...
        body: {
          error:
            "Access denied. You are not allowed to access this course content.",
        },
      },
    };
  }
  ...
  if (!chapter || String(chapter.courseId) !== String(courseId)) {
    return {
      error: {
        status: 400,
        body: { error: "Unit does not belong to this course" },
      },
    };
  }
  ...
  try {
    ...
    if (!canAccessCourseContent(course, req.user)) {
      return res
        ...
        .send({
          error:
            "Access denied. You are not allowed to view this course content.",
        });
    }
    ensureInstructorFallback(course);
    res.send(course);
  ...
  async (req, res) => {
    try {
      ...
      if (text.length < 12) {
        return res
          .status(400)
          .send({ error: "Feedback must be at least 12 characters long." });
      }
      if (text.length > 600) {
        return res
          .status(400)
          .send({ error: "Feedback must be 600 characters or less." });
      }
      if (!isValidHalfStepRating(overallRating)) {
        return res
          .status(400)
          .send({
            error: "Overall rating must be between 0 and 5 in 0.5 steps.",
          });
      }
      ...
      if (
        ...
        return res
          ...
          .send({
            error:
              "Access denied. You are not allowed to submit feedback for this course.",
          });
      }
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .send({
            error:
              "Access denied. You are not allowed to delete feedback for this course.",
          });
      }
...
router.delete(
  "/:courseId/testimonials/:testimonialId",
  auth,
    ...
    try {
      ...
      if (!canManageCourse({ ...course, _id: req.params.courseId }, req.user)) {
        return res
          ...
          .send({
            error:
              "Access denied. You are not allowed to moderate feedback for this course.",
          });
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      const lastCourseByOrder = await Course.findOne()
        .sort({ courseOrder: -1, createdAt: -1 })
        .select("courseOrder")
        .lean();
      const nextCourseOrder = Number.isFinite(lastCourseByOrder?.courseOrder)
        ? lastCourseByOrder.courseOrder + 1
        : 0;
  ...
  async (req, res) => {
    try {
      ...
      const isAssigned =
        ...
        course.assignedTeachers.some((t) => t.equals(req.user._id));
      ...
      if (!isAdmin && !isInstructor && !isAssigned) {
        return res
          ...
          .send({
            error:
              "Access denied. You do not have permission to edit this course.",
          });
      }
...
router.patch("/admin/reorder", auth, authorize("admin"), async (req, res) => {
  try {
    ...
    if (!Array.isArray(courseIds) || courseIds.length === 0) {
      return res
        .status(400)
        .send({ error: "courseIds must be a non-empty array" });
    }
    ...
    if (existingCourses.length !== courseIds.length) {
      return res
        .status(400)
        .send({ error: "Course order contains invalid course IDs" });
    }
    ...
    if (uniqueIds.size !== courseIds.length) {
      return res
        .status(400)
        .send({ error: "Course order contains duplicate IDs" });
    }
  ...
  try {
    ...
    if (!isAdmin && !isInstructor && !isAssigned) {
      return res
        ...
        .send({
          error:
            "Access denied. You do not have permission to delete this course.",
        });
    }
    ...
    try {
      ...
      if (!canManageCourse(course, req.user)) {
        return res
          ...
          .send({
            error:
              "Access denied. You do not have permission to add chapters to this course.",
          });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!Array.isArray(chapterIds) || chapterIds.length === 0) {
        return res
          .status(400)
          .send({ error: "chapterIds must be a non-empty array" });
      }
      ...
      if (!isAdmin && !isInstructor && !isAssigned) {
        return res
          ...
          .send({
            error:
              "Access denied. You do not have permission to reorder chapters for this course.",
          });
      }

      const existingChapterIds = (course.chapters || []).map((chapterId) =>
        String(chapterId),
      );
      const requestedChapterIds = chapterIds.map((chapterId) =>
        String(chapterId),
      );
      ...
      if (existingChapterIds.length !== requestedChapterIds.length) {
        return res
          .status(400)
          .send({ error: "Invalid chapter order payload length" });
      }
      ...
      for (const chapterId of requestedSet) {
        if (!existingSet.has(chapterId)) {
          return res
            .status(400)
            .send({ error: "Chapter order contains invalid chapter IDs" });
        }
      }
...
router.patch(
  ...
  async (req, res) => {
    try {
      const existingChapter = await Chapter.findById(req.params.id).select(
        "courseId title moduleDescriptionPdf moduleImage",
      );
      if (!existingChapter)
        return res.status(404).send({ error: "Chapter not found" });

      const course = await Course.findById(existingChapter.courseId).select(
        "instructor assignedTeachers",
      );
      if (!course) return res.status(404).send({ error: "Course not found" });
      if (!canManageCourse(course, req.user)) {
        return res
          ...
          .send({
            error:
              "Access denied. You do not have permission to update this chapter.",
          });
      }
      ...
      const chapter = await Chapter.findByIdAndUpdate(
        req.params.id,
        {
          ...
          moduleDescriptionPdf:
            req.body.moduleDescriptionPdf !== undefined
              ? req.body.moduleDescriptionPdf
              : existingChapter.moduleDescriptionPdf,
          moduleImage:
            req.body.moduleImage !== undefined
              ? req.body.moduleImage
              : existingChapter.moduleImage,
        },
        { new: true },
      );
      if (!chapter) return res.status(404).send({ error: "Chapter not found" });
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const course = await Course.findById(chapter.courseId).select(
        "instructor assignedTeachers",
      );
      if (!course) return res.status(404).send({ error: "Course not found" });
      if (!canManageCourse(course, req.user)) {
        return res
          ...
          .send({
            error:
              "Access denied. You do not have permission to delete this chapter.",
          });
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      const chapter = await Chapter.findById(req.params.chapterId).select(
        "courseId",
      );
      if (!chapter) return res.status(404).send({ error: "Chapter not found" });
      ...
      if (!canManageCourse(course, req.user)) {
        return res
          ...
          .send({
            error:
              "Access denied. You do not have permission to add units to this chapter.",
          });
      }
...
router.patch(
  "/chapters/:chapterId/units/reorder",
  auth,
  authorize("admin", "teacher"),
  async (req, res) => {
    try {
      ...
      if (!Array.isArray(unitIds) || unitIds.length === 0) {
        return res
          .status(400)
          .send({ error: "unitIds must be a non-empty array" });
      }
      ...
      if (!isAdmin && !isInstructor && !isAssigned) {
        return res
          ...
          .send({
            error:
              "Access denied. You do not have permission to reorder units for this chapter.",
          });
      }

      const existingUnitIds = (chapter.units || []).map((unitId) =>
        String(unitId),
      );
      ...
      if (existingUnitIds.length !== requestedUnitIds.length) {
        return res
          .status(400)
          .send({ error: "Invalid unit order payload length" });
      }
      ...
      for (const unitId of requestedSet) {
        if (!existingSet.has(unitId)) {
          return res
            .status(400)
            .send({ error: "Unit order contains invalid unit IDs" });
        }
      }
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const existingUnit = await Unit.findById(req.params.id).select(
        "chapterId type",
      );
      if (!existingUnit)
        return res.status(404).send({ error: "Unit not found" });

      const chapter = await Chapter.findById(existingUnit.chapterId).select(
        "courseId",
      );
      if (!chapter) return res.status(404).send({ error: "Chapter not found" });
      ...
      if (!canManageCourse(course, req.user)) {
        return res
          ...
          .send({
            error:
              "Access denied. You do not have permission to update this unit.",
          });
      }
      ...
      if (title !== undefined) {
        updatePayload.title =
          updatedType === "pdf" ? sanitizePdfUnitTitle(title) : title;
      }
      ...
      const unit = await Unit.findByIdAndUpdate(req.params.id, updatePayload, {
        new: true,
      });
    ...
    try {
      ...
      if (!canManageCourse(course, req.user)) {
        return res
          ...
          .send({
            error:
              "Access denied. You do not have permission to delete this unit.",
          });
      }

      await Unit.findByIdAndDelete(req.params.id);
      await Chapter.findByIdAndUpdate(unit.chapterId, {
        $pull: { units: req.params.id },
      });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const isAssigned =
        course.assignedTeachers &&
        course.assignedTeachers.some((t) => String(t) === String(req.user._id));
      const isAdmin = req.user.role === "admin";
      ...
      if (!enrollment)
        return res
          .status(400)
          .send({ error: "Student has not requested enrollment" });
...
router.delete(
  "/:courseId/assigned-teachers/:teacherId",
  auth,
  authorize("admin"),
  async (req, res) => {
    try {
      ...
      const assignedTeacherIds = (course.assignedTeachers || []).map((id) =>
        String(id),
      );
      if (!assignedTeacherIds.includes(String(teacherId))) {
        return res
          .status(404)
          .send({ error: "Teacher is not assigned to this course" });
      }

      const remainingAssigned = assignedTeacherIds.filter(
        (id) => id !== String(teacherId),
      );

      // Ensure course keeps a valid primary instructor.
      if (
        course.instructor &&
        String(course.instructor) === String(teacherId)
      ) {
        if (remainingAssigned.length === 0) {
          return res.status(400).send({
            error:
              "Cannot unassign the primary instructor without assigning another teacher first",
          });
        }
  ...
  async (req, res) => {
    try {
      ...
      const students = await User.find(
        {
          enrolledCourses: {
            $elemMatch: { course: course._id, status: "pending" },
          },
        },
        "username _id enrolledCourses",
...
router.post("/:courseId/unit/:unitId/like", auth, async (req, res) => {
  try {
    const accessResult = await getAccessibleCourseAndUnit(
      req.params.courseId,
      req.params.unitId,
      req.user,
    );
...
router.get("/:courseId/unit/:unitId/comments", auth, async (req, res) => {
  try {
    ...
    if (unit.type !== "video") {
      return res
        .status(400)
        .send({ error: "Comments are only supported for video units" });
    }
...
router.post("/:courseId/unit/:unitId/comments", auth, async (req, res) => {
  try {
    ...
    if (cleanText.length > 1000) {
      return res
        .status(400)
        .send({ error: "Comment must be at most 1000 characters" });
    }
...
// Edit own comment
router.patch(
  "/:courseId/unit/:unitId/comments/:commentId",
  auth,
  async (req, res) => {
    try {
      const { text } = req.body;
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const isAdmin = req.user.role === "admin";
      const isAssociatedTeacher = isAssociatedTeacherForCourse(
        course,
        req.user,
      );
