...
const isInstructorProfileComplete = (profile = {}) => {
  ...
  const hasStats = ["experience", "publications", "patents", "startups"].every(
    (key) => String(stats[key] || "").trim().length > 0,
  );
...
router.post("/toggle-hidden-content", auth, async (req, res) => {
  try {
    ...
    if (!user) {
      logger.error("User not found in toggle-hidden-content", {
        userId: req.user._id,
      });
    ...
    if (!enrollment) {
      const available = user.enrolledCourses
    ...
    const index = enrollment.hiddenContent.findIndex(
      (id) => id && id.toString() === contentId,
    );
    ...
    logger.error("Error in toggle-hidden-content", {
      error: e.message,
      stack: e.stack,
    });
...
router.post("/login", validate(loginSchema), async (req, res) => {
  try {
    ...
    if (!isPasswordValid) {
      logger.error("User authentication failed: invalid password", {
        identifier: username,
        role: user.role,
      });
      return res.status(400).send({ error: "Invalid login credentials" });
    ...
    if (user.isFrozen) {
      return res
        .status(403)
        .send({ error: "Account is frozen by admin. Contact support." });
    }
    ...
    try {
      ...
      if (!profile.isProfileComplete) {
        return res.status(400).send({
          error:
            "Complete all required profile fields before publishing to the Professor tab.",
          profile,
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!firstName || !lastName) {
        return res
          .status(400)
          .send({ error: "First and Last name are required" });
      }
        ...
        if (existing.status === "completed") {
          ...
          if (!user) {
            ...
            return res
              ...
              .send({
                error:
                  "A request for this name already exists and the student is active.",
              });
          }
          ...
          return res
            ...
            .send({
              error:
                "A request for this name already exists and is being processed.",
            });
        }
      }
  ...
  async (req, res) => {
    try {
      ...
      await createNotification({
        ...
        message:
          "Your account has been unfrozen by admin. You can now continue learning.",
        link: "/student",
      });
  ...
  async (req, res) => {
    try {
      ...
      await createNotification({
        ...
        message:
          "Your account has been frozen by admin. Contact support for help.",
        link: "/login",
      });
  ...
  try {
    ...
    if (user.isFrozen) {
      return res.status(403).send({
        error:
          "Your account is frozen by admin. Password reset is disabled while account is frozen.",
      });
    }
...
router.post("/reset-password", authLimiter, async (req, res) => {
  try {
    ...
    if (user.isFrozen) {
      return res.status(403).send({
        error:
          "Your account is frozen by admin. You cannot reset password until the account is unfrozen.",
      });
    }
