...
import { fetchMyInstructorProfile, updateMyInstructorProfile } from '../api/api';
...
const InstructorProfile = () => {
    ...
    const requiredMissing = useMemo(() => {
        ...
        if ((form.bio || '').trim().length < 50) missing.push('Bio (min 50 characters)');
    ...
    useEffect(() => {
        const loadProfile = async () => {
            try {
                ...
                showToast.error(error.message || 'Failed to load instructor profile');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSave = async (event) => {
        ...
        try {
            ...
            showToast.error(error.message || 'Please complete all required fields');
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <PageLayout title="Instructor Profile">
            <div style={{ maxWidth: '900px', margin: '0 auto', display: 'grid', gap: spacing.lg }}>
                <Card>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: spacing.md, alignItems: 'center', flexWrap: 'wrap' }}>
                        <div>
                            <h2 style={{ ...typography.h3, margin: 0 }}>Instructor Profile</h2>
                            <p style={{ ...typography.small, margin: `${spacing.xs} 0 0 0`, color: colors.textMuted }}>
                                Complete all required fields before your profile appears in the Professor tab.
                            </p>
                        </div>
                ...
                <Card>
                    <div style={{ display: 'grid', gap: spacing.md }}>
                        <div style={{ ...typography.label, color: isComplete ? colors.success : colors.danger }}>
                            {isComplete
                ...
                <Card>
                    <form onSubmit={handleSave} style={{ display: 'grid', gap: spacing.md }}>
                        ...
                        <div style={{ display: 'grid', gap: spacing.xs }}>
                            <label style={typography.label}>Bio * (minimum 50 characters)</label>
                            <textarea
                                value={form.bio}
                            ...
                            />
                            <span style={{ ...typography.xsmall, color: colors.textMuted }}>
                                {form.bio.trim().length} characters. Use blank lines to create paragraphs.
                            </span>
                        </div>

                        <div style={{ ...typography.label, marginTop: spacing.sm }}>Stats (required)</div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: spacing.md }}>
                            <Input
                                label="Years Experience *"
                                value={form.stats.experience}
                                onChange={(e) => setForm({ ...form, stats: { ...form.stats, experience: e.target.value } })}
                                required
                            />
                            <Input
                                label="Publications *"
                                value={form.stats.publications}
                                onChange={(e) => setForm({ ...form, stats: { ...form.stats, publications: e.target.value } })}
                                required
                            />
                            <Input
                                label="Patents *"
                                value={form.stats.patents}
                                onChange={(e) => setForm({ ...form, stats: { ...form.stats, patents: e.target.value } })}
                                required
                            />
                            <Input
                                label="Startups *"
                                value={form.stats.startups}
                                onChange={(e) => setForm({ ...form, stats: { ...form.stats, startups: e.target.value } })}
                                required
                            />
                        </div>

                        <div style={{ ...typography.label, marginTop: spacing.sm }}>Contact</div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: spacing.md }}>
                            <Input
                                label="Email"
