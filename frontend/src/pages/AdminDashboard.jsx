...
import { Plus, Users, Book, Trash2, Edit, X, Layout, User, Eye } from 'lucide-react';
import { fetchCourses, createCourse, deleteCourse, addTeacher, fetchTeachers, deleteTeacher, updateCourse, fetchEnrolledStudents, fetchStudents, deleteStudent } from '../api/api';
...
const AdminDashboard = () => {
    ...
    const [view, setView] = useState('courses'); // 'courses', 'teachers', 'students'
            ...
            {view === 'courses' ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem' }}>
                    {students.map(student => (
                        <div key={student._id} className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem', justifyContent: 'space-between' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                <div style={{ background: 'var(--glass)', padding: '1rem', borderRadius: '50%' }}>
