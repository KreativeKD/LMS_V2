...
const CourseEditor = () => {
  ...
  const [editingChapterPdfFileName, setEditingChapterPdfFileName] =
    useState("");
  const [editingChapterImageFileName, setEditingChapterImageFileName] =
    useState("");
  const [editingChapterPdfChanged, setEditingChapterPdfChanged] =
    useState(false);
  const [editingChapterImageChanged, setEditingChapterImageChanged] =
    useState(false);
  ...
  const handleAddChapter = async (e) => {
    ...
    try {
      ...
      handleApiError(err, "Failed to add chapter");
    }
  };
  ...
  const handleAddChapterPdfUpload = (event) => {
    ...
    reader.onload = () => {
      ...
      setChapterForm((prev) => ({
        ...prev,
        moduleDescriptionPdf: fileDataUrl,
      }));
  ...
  const getReorderedChapters = (chapters, fromChapterId, toChapterId) => {
    const fromIndex = chapters.findIndex(
      (chapter) => chapter._id === fromChapterId,
    );
    const toIndex = chapters.findIndex(
      (chapter) => chapter._id === toChapterId,
    );
  ...
  const handleChapterDrop = async (targetChapterId) => {
    if (
      !course ||
      !draggedChapterId ||
      draggedChapterId === targetChapterId ||
    ...
    const reordered = getReorderedChapters(
      currentChapters,
      draggedChapterId,
      targetChapterId,
    );
  ...
  const handleUnitDrop = async (chapterId, targetUnitId) => {
    if (
      !course ||
      !draggedUnitInfo ||
      draggedUnitInfo.chapterId !== chapterId ||
      isReorderingUnits
    ) {
    ...
    const reorderedUnits = getReorderedUnits(
      currentUnits,
      draggedUnitInfo.unitId,
      targetUnitId,
    );
    ...
    try {
      await reorderUnits(
        chapterId,
        reorderedUnits.map((unit) => unit._id),
      );
  ...
  const handleUnitSubmit = async (e) => {
    ...
    try {
      ...
      handleApiError(err, "Failed to add unit");
    }
  };
  ...
  const handleEditChapter = (chapter) => {
    ...
    setEditingChapterPdfFileName(
      chapter.moduleDescriptionPdf ? "Current uploaded PDF" : "",
    );
    setEditingChapterImageFileName(
      chapter.moduleImage ? "Current uploaded image" : "",
    );
    setEditingChapterPdfChanged(false);
    setEditingChapterImageChanged(false);
  ...
  const handleUpdateChapter = async (e) => {
    e.preventDefault();
    try {
      const payload = { title: editingChapter.title };

      if (editingChapterPdfChanged) {
        payload.moduleDescriptionPdf =
          editingChapter.moduleDescriptionPdf || "";
      }

      if (editingChapterImageChanged) {
        payload.moduleImage = editingChapter.moduleImage || "";
      }

      await updateChapter(editingChapter._id, payload);
      ...
      handleApiError(err, "Failed to update chapter");
    }
  };
  ...
  const handleChapterPdfUpload = (event) => {
    ...
    reader.onload = () => {
      ...
      setEditingChapterPdfChanged(true);
    };
    reader.onerror = () => showToast.error("Failed to read PDF file");
    reader.readAsDataURL(file);
  };
  ...
  const handleChapterImageUpload = (event) => {
    ...
    reader.onload = () => {
      ...
      setEditingChapterImageChanged(true);
    };
    reader.onerror = () => showToast.error("Failed to read image file");
    reader.readAsDataURL(file);
  };
  ...
  const handleEditUnit = (unit) => {
    setEditingUnit({
      ...unit,
      contentValue:
        unit.content?.videoUrl ||
        unit.content?.pdfUrl ||
        unit.content?.text ||
        "",
    });
    setEditingPdfFileName(
      unit.type === "pdf" && unit.content?.pdfUrl ? "Current uploaded PDF" : "",
    );
  ...
  const handleUpdateUnit = async (e) => {
    ...
    try {
      const content = {};
      if (editingUnit.type === "video")
        content.videoUrl = editingUnit.contentValue;
      else if (editingUnit.type === "pdf")
        content.pdfUrl = editingUnit.contentValue;
      ...
      handleApiError(err, "Failed to update unit");
    }
  };
  ...
  const handleQuizSubmit = async (e) => {
    ...
    try {
      ...
      handleApiError(err, "Failed to create quiz");
    }
  };
  ...
  const handleUpdateQuiz = async (e) => {
    ...
    try {
      ...
      handleApiError(err, "Failed to update quiz");
    }
  };
  ...
  const handleSaveCourseType = async () => {
    ...
    try {
      const payload = {
        ...
        contentHours: Number.isFinite(course.contentHours)
          ? course.contentHours
          : 0,
        image: course.image || "",
  ...
  if (loading)
    return (
      <PageLayout title="Course Editor">
        ...
        >
          ...
          >
            ...
            <Button
              ...
              disabled={
                (course.courseType || "academic") === selectedCourseType
              }
            >
        ...
        <section>
          ...
          >
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    borderBottom: expandedChapters[chapter._id]
                      ? `1px solid ${colors.border}`
                      : "none",
                  }}
                >
                ...
                {expandedChapters[chapter._id] && (
                  ...
                  >
                    ...
                    >
                      {chapter.units?.map((unit) => (
                        <div
                          ...
                          onDragStart={() =>
                            handleUnitDragStart(chapter._id, unit._id)
                          }
                          onDragOver={(e) => {
                            e.preventDefault();
                          ...
                          style={{
                            ...
                            borderRadius: "6px",
                            border:
                              dragOverUnitInfo?.chapterId === chapter._id &&
                              dragOverUnitInfo?.unitId === unit._id
                                ? `1px solid ${colors.accent}`
                            ...
                            boxShadow:
                              ...
                              dragOverUnitInfo?.unitId === unit._id
                                ...
                                : "none",
                            opacity:
                              draggedUnitInfo?.unitId === unit._id ? 0.6 : 1,
                          }}
                        >
                          <GripVertical size={14} color={colors.textMuted} />
                          {unit.type === "video" ? (
                            <VideoIcon size={16} color="var(--primary)" />
                          ) : unit.type === "pdf" ? (
                          ...
                          <span
                            ...
                            style={{
                              color:
                                unit.type === "quiz" ? "var(--accent)" : "gray",
                            }}
                          >
                            {unit.type}
                          </span>
                    ...
                    >
                      <Button
                        onClick={() => openUnitModal(chapter._id, "video")}
                        variant="secondary"
                        size="sm"
                      >
                      ...
                      <Button
                        onClick={() => openUnitModal(chapter._id, "pdf")}
                        variant="secondary"
                        size="sm"
                      >
                      ...
                      <Button
                        onClick={() => openUnitModal(chapter._id, "text")}
                        variant="secondary"
                        size="sm"
                      >
                        <Save size={14} /> +Text
                      ...
                      <Button
                        onClick={() => openUnitModal(chapter._id, "quiz")}
                        variant="primary"
                        size="sm"
                      >
            ...
            {(!course.chapters || course.chapters.length === 0) && (
              ...
              >
                No chapters created yet. Start building your curriculum!
              </p>
            )}
          </div>
        </section>
            ...
            >
              ...
              >
                <h2 className="gradient-text">
                  Add {unitType.charAt(0).toUpperCase() + unitType.slice(1)}{" "}
                  Unit
                </h2>
                <Button
                  onClick={() => {
                  ...
                  {unitType === "text" ? (
                    ...
                    >
                      <input
                        ...
                        accept="application/pdf"
                        onChange={(event) =>
                          handlePdfFileUpload(event, "create")
                        }
                        required
                        style={{
                          background: "#f9fafb",
        ...
        {showQuizModal && (
          <div className="modal-overlay">
            <div className="modal-content animate-fade-in">
              ...
              >
                <h2 className="gradient-text">
                  {editingQuiz ? "Edit MCQ Quiz" : "Create MCQ Quiz"}
                </h2>
                <Button
                  onClick={() => {
                    setShowQuizModal(false);
              ...
              <form
                onSubmit={editingQuiz ? handleUpdateQuiz : handleQuizSubmit}
              >
                <Input
                ...
                >
                  {quizData.questions.map((q, qIdx) => (
                    ...
                    >
                      ...
                      >
                        <h4 style={{ color: "var(--text-accent)", margin: 0 }}>
                          Question {qIdx + 1}
                        </h4>
                        <button
                          type="button"
          ...
          <div className="modal-overlay">
            ...
            >
              ...
              >
                <h2 className="gradient-text">
                  Edit{" "}
                  {editingUnit.type.charAt(0).toUpperCase() +
                    editingUnit.type.slice(1)}{" "}
                  Unit
                </h2>
                <Button
                  onClick={() => {
                ...
                >
                  ...
                  {editingUnit.type === "text" ? (
                    ...
                    >
                      ...
                      >
                        {editingPdfFileName
                          ? `Selected: ${editingPdfFileName}`
                          : "No new PDF selected (keeps current PDF)"}
                      </span>
                    </div>
                  ) : (
                    <Input
