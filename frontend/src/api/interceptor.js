...
export const handleApiError = (error, customMessage = null) => {
    ...
    if (!error.response) {
        if (error.message === 'Failed to fetch') {
            showToast.error(customMessage || 'Network connection error. Please check your internet.');
        } else {
    ...
    switch (status) {
        ...
        case 401:
            ...
            break;
        case 403:
            showToast.error(customMessage || 'You do not have permission for this action.');
            break;
        case 404:
        ...
        case 504:
            ...
            break;
        default:
            showToast.error(customMessage || message || 'An error occurred. Please try again.');
    }
};
...
export const createErrorFromResponse = (status, data) => {
    const error = new Error(data?.error?.message || data?.error || data?.message || 'Unknown error');
    error.response = {
        status,
        data
    };
