...
const Navbar = () => {
  ...
  return (
    <nav
      className={`nav-bar unified-navbar ${onPublicPage ? `public-navbar ${scrolled ? "scrolled" : ""}` : "auth-navbar"}`}
    >
      <div
        className="nav-logo"
      ...
      <div className="nav-links">
        ...
        <Link
          to="/professor"
          className={`nav-link ${isPathActive("/professor") ? "active" : ""}`}
        >
          Instructor
        </Link>
        <a
          href="/#testimonials"
          className={`nav-link ${isTestimonialsActive ? "active" : ""}`}
        ...
        {isAuthenticated && (
          <>
            <Link
              to={getDashboardPath()}
              className={`nav-link auth-nav-link ${isPathActive(getDashboardPath()) ? "active" : ""}`}
            >
              Courses
            </Link>
            ...
            {user?.role === "student" && (
              <Link
                to="/my-courses"
                className={`nav-link auth-nav-link ${isPathActive("/my-courses") ? "active" : ""}`}
              >
                My Courses
              </Link>
            )}
            ...
            {(user?.role === "admin" || user?.role === "teacher") && (
              <Link
                to="/teacher"
                className={`nav-link auth-nav-link ${isPathActive("/teacher") ? "active" : ""}`}
              >
                Teaching
              </Link>
            )}
      ...
      <div
        className={`nav-actions ${isAuthenticated ? "auth-nav-actions" : ""}`}
      >
        {!isAuthenticated ? (
          <button
            className="btn-primary public-login-btn"
            ...
            <div ref={notificationRef} style={{ position: "relative" }}>
              ...
              {isOpen && (
                <div className="auth-notification-panel">
                  ...
                  {loading && (
                    <div
                      style={{ padding: "0.9rem", color: "var(--text-muted)" }}
                    >
